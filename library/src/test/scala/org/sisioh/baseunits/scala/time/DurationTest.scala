...
object DurationTest extends Assertions {
  ...
  def test12_Divide {
    assert(
      Duration
        .days(3)
        .dividedBy(Duration.days(2))
        .decimalValue(1, BigDecimal.RoundingMode.DOWN) == BigDecimal(
        1.5
      ))
  }
  ...
  def test13_ToNormalizedString {
    ...
    assert(
      complicatedDuration.toNormalizedString == "5 days, 4 hours, 3 minutes, 2 seconds, 1 millisecond")
  ...
  def test2o_AddToCalendarMonth {
    ...
    assert(
      threeHours.addedTo(CalendarYearMonth.from(2010, 11, ZoneIds.Default), ZoneIds.Default) == CalendarYearMonth
        .from(2010, 11))
  }
